...
1. 個人情報・NG語・自傷・他害・マサカリのいずれかを検出した場合は`block`
2. それ以外は`allow`（変換結果が150文字を超えた場合のみ`rewrite_required`）

この判定は形態素解析と辞書だけで行い、外部の推論APIを呼ばない。そのため利用回数の制限を受けず、変換用の推論APIが停止していても動作する。利用者が自分で赤ちゃん語を書いた場合も、そのまま投稿できる。

**文章が赤ちゃん語・ママ語の書き方になっているかは、この判定では見ない。** 人間監督の決定により、採点基準をこちらで決めない。ラベル付きの学習データから得られる判断と食い違うためである（例：お母さんの文章に絵文字や顔文字を必須にしてしまう）。文体の判定は、学習データに基づく方式で後から入れる。そのため`FR-AI-TRANS-004`（原文のままでは利用できないことを利用者に伝える）は、現時点では未実装である。
...
- 判定の内訳（どのNG語に当たったか）は返さない（FR-MOD-034）。
- **書き方が赤ちゃん語・ママ語になっているかは判定しない**（上記のとおり、採点基準を置かない）。
...
- JWTの有効期限、リフレッシュトークンの要否。ステートレスであるため即時失効ができない点をどこまで許容するか（例：有効期限を短くする、ブロックリストを別途持つ等）。
- パスワードのハッシュ化方式（bcrypt / scrypt / Argon2id 等）、パスワードの最小長・文字種要件、パスワード確認欄の要否、パスワードを忘れた場合の復旧手段（メールアドレスを取らない仕様のため、復旧手段が無い前提でよいか）。
- ログインID・生年月日のバリデーション規則（文字種・長さ、生年月日の未来日付や極端な値の扱い）。
