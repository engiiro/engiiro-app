...
```mermaid
flowchart TD
    FE["フロントエンド<br/>React + TypeScript + Vite<br/>(Vercelにデプロイ)"]
    BE["バックエンドAPI<br/>TypeScript / Deno<br/>(Deno Deployにデプロイ)"]
    DB[("PostgreSQL<br/>(RDB)")]
    AI["AI推論API<br/>Python / FastAPI<br/>Gemini API + ルールベースフォールバック<br/>(Cloud Runにデプロイ)"]
...
- **フロントエンド**：React + TypeScript + Viteで実装し、Vercelにデプロイする（2026-08決定。本ドキュメントではスコープ外）。未ログイン時は認証トークンを送らずに閲覧系APIのみを呼び出す（4.4参照）。
...
- **データベース**：**PostgreSQL（RDB）**。当初はDeno KV（キーバリューストア）を前提にしていたが、リレーショナルなPostgreSQLが利用できる環境が整ったため、こちらを採用する（詳細は6章）。ホスティングはDeno Deployが提供するPostgres（`backend/CLAUDE.md`参照）。具体的な接続方式（コネクションプーラーの要否等）・ORM／クエリビルダの選定は`npm:pg`を直接使う形で決着している（`backend/src/lib/db.ts`）。
- **AI処理**：文章が「赤ちゃんらしいか／お母さんらしいか」を年齢としてスコアリングする**評価**（`/api/ai/evaluate`）と、文章を赤ちゃん言葉・お母さん言葉に書き換える**変換**（`/api/ai/transform`）の2種類を、バックエンドAPIから独立した推論APIとして呼び出す。フロントエンドから推論APIを直接呼び出すことはしない（FR-AI-002）。**変換**はGemini API（一次経路）とルールベースの辞書変換（フォールバック経路）の2段構えに決定済み（`ai/README.md`、`docs/ai_transform_design.md`）。**評価**の`passesThreshold`判定はナイーブベイズ3クラス分類器（フルスクラッチ実装、`ai/src/style_classifier.py`）に決定済み。AI推論API自体はFastAPIで実装しGoogle Cloud Run（Hugging Face Spacesではない）にデプロイしている。
...
- `estimatedAge`（年齢の目安）の算出方法。現状は`degree_of()`が返す確信度（0〜100）を0〜72ヶ月へ線形変換する簡易ロジック（`ai/src/evaluate.py`、2026-08-28決定）で、より精緻なモデルへの置き換えは未検討。
- 未ログインユーザーに興味関心・技術領域を入力させてタイムラインをレコメンドする拡張機能（3.6参照）の詳細。
- 本格運用を見据えた場合のスケーラビリティ・モデレーション体制の検討。

以下は本ドキュメントの旧版でオープンイシューとして記載していたが、実装により決着済みのため削除した（2026-09-18整理）：PostgreSQLのホスティング先・接続方式・ORM選定（Deno DeployのPostgres＋`npm:pg`に決定）、生成AI変換の実装方式（Gemini API＋ルールベースフォールバックに決定）、モデレーションの具体的な実装（`ai/moderation_rules.py`に決定）、フロントエンドの技術選定（React + TypeScript + Viteに決定）。
