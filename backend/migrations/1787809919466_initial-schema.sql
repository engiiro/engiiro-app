...
-- design_doc.md本体ではidをuuidにしているが、ローカルE2E検証ではfrontend
-- （engiiro-app-feat-ui-profile/frontend/app/src/components/BubbleBody.tsx の
-- StampShape）が "nemui" "ogya" 等の固定スラッグでスタンプの絵柄を分岐している。
-- UUIDにすると絵柄が解決できず見た目のE2E確認ができないため、スタンプのみ
-- varcharのスラッグ主キーに変更する（他テーブルのPK設計とは意図的に異なる）。
create table stamps (
...
create table post_stamps (
    ...
    stamp_id varchar(30) not null references stamps (id),
    position integer
);
...
drop table if exists persona_age_estimates;
drop table if exists follows;
drop trigger if exists reactions_limit_check on reactions;
drop function if exists reactions_enforce_limit();
drop table if exists reactions;
drop table if exists comments;
drop table if exists post_stamps;
drop table if exists posts;
drop table if exists stamps;
drop table if exists mother_personas;
drop table if exists baby_personas;
