...
| 対象 | 使えるリアクション |
|---|---|
| バブル | `おぎゃー` / `よしよし` / `まんま` の3種 |
| 赤ちゃんとしてのあやす | 同じ3種 |
| お母さんとしてのあやす | **`ばぶー` の1種のみ** |

- **旧称の「わかるわぁ」「哺乳瓶」は `まんま` に統一された。2種類として実装しない**（FR-REACT-009）
- 同じ人が**1種類につき5回まで**押せる。自分のバブルには押せない（人間の決定 2026-08-25）
...
- **AI 文章「生成」と「評価」で、止まったときの扱いが逆**（PO 回答 2026-08-25、Issue #19）
  - 生成が止まっても、バブルの投稿とあやすは続けられる（NFR-001）
  - 評価が止まったら、投稿を妨げる（NFR-003）。閾値を通ったものだけを保存する（FR-AI-EVAL-007）
- **AI 文章評価の閾値は backend が持つ。frontend に置かない**（PO 決定 2026-08-25）
- 評価の口は 2 つの場面で使う。**「はかる」ボタンでは合否を出さず、指標だけを見せる。**
  合否は保存時に backend が決める
