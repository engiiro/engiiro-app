...
- `stamps.json` の `imageUrl` が絵の場所です（設計書 §7 の `GET /api/stamps` と同じ項目）。
  実体は `public/images/stamps/<id>.png`（透過 PNG、128px の正方）。
  絵を差し替えるときは、同じ名前のファイルを置き換えれば画面も変わります
- `stamps.json` の `shelf` は、一覧を分ける棚です（人間の指示、2026-08-27 / 28）。
  使えるキーは `weak`（よわね）/ `glad`（うれしい）/ `soothe`（よしよし）/ `reply`（へんじ）。
  表に無いキーを書いたスタンプは、一覧の「そのほか」に入ります（消えはしません）。
  **`shelf` は `GET /api/stamps` の応答には無い項目です。** 実 API に差し替えるときは backend 側の追加が要ります
- **本文にスタンプを入れるときは `:naku:` のように id を `:` で囲みます。**
  カタログに無い id はただの文字として出ます。文字数では絵1つ＝1文字ぶん（FR-POST-002）
