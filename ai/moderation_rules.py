...
**通信しない。** 判定APIの中身はここだけで動く。

語単位で表れないマサカリ（「なんでこんなコード書いたの。ありえないんだけど。」）は
ここでは拾えない。埋め方は辞書を育てることである。
LLMは判定に使わず、辞書へ入れる語を探す道具として別に置いてある
（transform_api.moderate_by_llm）。
...
# **4つの辞書はどれも block になる。** 分かれているのは理由コードのためで、
# 弾いたあと利用者へ何を見せるかが変わる（自傷なら相談窓口を出す、など）。
# 「言い換えられるか」で分ける基準は、もう使っていない。
...
# いずれも実測で確認済み。文脈を見ないと判定できないため、**拾えない。**
# 判定APIにLLMはいないので、誰かが拾ってくれるわけではない。
# 埋めるなら、日常語と衝突しない別の言い回しを辞書へ足すことになる。
...
# 学校・勤務先の具体名（仕様書 FR-MOD-013）。
#
# FR-MOD-020（組み合わせによる特定）の一部にあたるが、**満たしてはいない。**
# あちらは複数の文にまたがる組み合わせを求めており、1文ずつの判定では追えない。
#
# **MVPでは実装しない。人間監督の決定である。**
#
#   「AでもBでも結局のところ一度は通すことになるなら、Cで行きます。
#     そこはインターン後にゆっくり考えます」
#
# 検討した案は3つだった。
#   A 判定APIへ過去の投稿も渡す
#   B 保存する側で、同じ人の投稿をまとめて見る
#   C MVPでは実装しない  ← これ
#
# **AもBも、組み合わせが成立する最後の1件を投稿するまで検出できない。**
# その時点で、特定に足りる情報はすでに出そろって公開されている。
# 後から弾いても手遅れなので、MVPで作り込む価値が薄いという判断である。
# 作り直すときは、そもそも「投稿してから弾く」以外の形を考えること。
#
...
def find_organizations(text: str) -> list[str]:
    """学校名・勤務先名らしき並びを探す。仕様書 FR-MOD-013 に対応する。

    FR-MOD-020（組み合わせによる特定）の一部でもあるが、**満たしてはいない。**
    複数の文にまたがる組み合わせは、1文ずつの判定では追えない。
    人間監督の決定によりMVPでは実装しない。理由は上の注記を読むこと。
...
def check_rules(text: str) -> dict:
    ...
    **返すのは "allow" か "block" だけである。**
    ...
    判定APIから rewrite_required は出ない。変換APIが、変換結果が
    150文字を超えたときにだけ付ける（理由コードは too_long）。
