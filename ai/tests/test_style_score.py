...
# ============================================================
# 生成APIが止まっているとき、自分で書いた文で投稿できること
# ============================================================
# 人間監督の意図：
#   「生成APIが止まった時のためにローカルで無限に使える判定のみで
#     動くようにするつもりなので、原文で通るようにしないと、
#     そもそも一日中使えなくなってしまいます」
#
# 利用者が自分で赤ちゃん語・ママ語を書いたとき、通信なしで allow が出ること。
# ここが落ちると、Gemini が止まった日は誰も投稿できなくなる。

HANDWRITTEN = {
    "baby": [
        "きょう おしごと がんばったのー。",
        "おなか すいたのー。ごはん たべたいのー。",
        "ねむいのー。もう ねちゃうもん。",
        "たしかめ できなくて こまってるのー。うぅ。",
        "おともだちと あそんだのー。たのしかったのー。えへへ。",
        "ぼく きょう いっぱい はたらいたのー。",
    ],
    "mother": [
        "よくがんばったね 😊",
...
@pytest.mark.parametrize(
    "mode,text",
    [(mode, text) for mode, texts in HANDWRITTEN.items() for text in texts],
    ids=lambda v: v[:18] if isinstance(v, str) and len(v) > 4 else str(v),
)
def test_手で書いた赤ちゃん語ママ語は通信なしでallow(mode, text, no_network_score):
    result = T.moderate(text, mode)
    assert result["score"] == 100, result["styleChecks"]
...
@pytest.mark.parametrize("text", [
    ...
    # 英字を使う顔文字。人間監督の決定により拾う
    "つらかったね (T_T)", "がんばったね (ToT)", "よしよし (>_<)",
    "そうなのね (;_;)", "わかるよ (^_^)", "おつかれ (´∀｀)", "ふむ (-_-)",
], ids=lambda t: t[-4:])
...
@pytest.mark.parametrize("text", [
    ...
    "直したね (abc)", "直したね (note)", "直したね (v2)", "直したね (bug)",
    "直したね (json)", "直したね (main)", "直したね (株)",
    # その他の記号
    "①をやったね", "5℃だったね", "✓ を付けたね",
    "つらかったね",
], ids=lambda t: t[:12])
